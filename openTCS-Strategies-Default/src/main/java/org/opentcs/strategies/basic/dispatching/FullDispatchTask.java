...
public class FullDispatchTask
  ...
  public final void run() {
    ...
    checkNewOrdersPhase.run();//新订单的过程
    // Check what vehicles involved in a process should do.
    finishWithdrawalsPhase.run();//完成订单的取消过程
    assignNextDriveOrdersPhase.run();//签约下一个订单
    assignSequenceSuccessorsPhase.run();//成功签约的订单的执行
    // Check what vehicles not already in a process should do.
    assignOrders();//签约订单
    rechargeVehicles();//充电车辆
    parkVehicles();//停止车辆
